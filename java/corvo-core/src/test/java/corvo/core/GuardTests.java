...
final class GuardTests {
    ...
    private static void cabecerasDeSeguridad() throws Exception {
        ...
        Server aMedida = Corvo.app().port(0).quiet().reporter(ErrorReporter.silent())
                .use(SecurityHeaders.standard()
    ...
    private static void cors() throws Exception {
        Server server = Corvo.app().port(0).quiet().reporter(ErrorReporter.silent())
        ...
        Server abierto = Corvo.app().port(0).quiet().reporter(ErrorReporter.silent())
                .use(Cors.anyOrigin())
                .controllers(ApiController.class)
                .start();
        try {
