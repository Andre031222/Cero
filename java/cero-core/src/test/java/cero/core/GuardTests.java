...
final class GuardTests {
    ...
    private static void cabecerasDeSeguridad() throws Exception {
        ...
        Server aMedida = Cero.app().port(0).quiet().reporter(ErrorReporter.silent())
                .use(SecurityHeaders.standard()
    ...
    private static void cors() throws Exception {
        Server server = Cero.app().port(0).quiet().reporter(ErrorReporter.silent())
        ...
        Server abierto = Cero.app().port(0).quiet().reporter(ErrorReporter.silent())
                .use(Cors.anyOrigin())
                .controllers(ApiController.class)
                .start();
        try {
    ...
    /** Comodín y credenciales a la vez deja leer las respuestas autenticadas a cualquiera. */
    private static void corsConCredenciales() throws Exception {
        Check.raises("anyOrigin con credenciales no se deja construir",
                IllegalStateException.class, () -> Cors.anyOrigin().credentials(true));
        Check.raises("allowing(\"*\") con credenciales tampoco",
                IllegalStateException.class, () -> Cors.allowing("*").credentials(true));

        Check.that("anyOrigin sin credenciales sigue valiendo",
                Cors.anyOrigin().credentials(false) != null);
        Check.that("y una lista concreta con credenciales también",
                Cors.allowing("https://app.pe").credentials(true) != null);

        // Con lista concreta el eco es correcto: la lista la puso quien configuró.
        Server credenciales = Cero.app().port(0).quiet().reporter(ErrorReporter.silent())
                .use(Cors.allowing("https://app.pe").credentials(true))
                .controllers(ApiController.class)
                .start();
        try {
            String base = "http://127.0.0.1:" + credenciales.port();
            HttpResponse<String> propio = send(base + "/api", "GET", "Origin", "https://app.pe");
            Check.equal("el origen listado se devuelve tal cual",
                    propio.headers().firstValue("access-control-allow-origin").orElse(null),
                    "https://app.pe");
            Check.equal("con credenciales",
                    propio.headers().firstValue("access-control-allow-credentials").orElse(null), "true");
            Check.that("y con Vary, porque la respuesta depende del origen",
                    propio.headers().firstValue("vary").orElse("").contains("Origin"));

            HttpResponse<String> ajeno = send(base + "/api", "GET", "Origin", "https://mala.pe");
            Check.equal("un origen no listado no recibe cabecera de permiso",
                    ajeno.headers().firstValue("access-control-allow-origin").orElse(null), null);
            Check.equal("ni de credenciales",
                    ajeno.headers().firstValue("access-control-allow-credentials").orElse(null), null);
        } finally {
            credenciales.stop();
        }
    }
    ...
    private static void csrf() throws Exception {
        ...
        try {
            ...
            Check.that("y una sesión", cookie.contains("CEROSESSION"));
    ...
    private static void sanitizado() {
        ...
        // Antes se recortaba el esquema y quedaba `href="malo()"`; ahora cae el atributo entero.
        Check.equal("elimina el protocolo javascript",
                Sanitize.html("<a href=\"javascript:malo()\">x</a>"), "<a>x</a>");
