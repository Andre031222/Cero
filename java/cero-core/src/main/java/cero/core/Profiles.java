 ...
 *   cero.profiles = prod,metricas
 * </pre>
 *
 * <p>Se puede pisar por propiedad del sistema (<code>-Dcero.profiles=dev</code>) o por entorno
 * (<code>CERO_PROFILES=dev</code>), en ese orden de prioridad. Sin nada configurado el perfil es
 * <code>default</code>.
 *
 * <pre>
...
public final class Profiles {
    ...
    /** Perfiles de la configuración, pisados por {@code -Dcero.profiles} y por {@code CERO_PROFILES}. */
    public static Profiles from(Config config) {
        String declarado = System.getProperty("cero.profiles");
        if (declarado == null) {
            declarado = System.getenv("CERO_PROFILES");
        }
        if (declarado == null && config != null) {
            declarado = config.get("cero.profiles", null);
        }
