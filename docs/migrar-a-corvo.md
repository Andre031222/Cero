# Migrar una aplicación de LuxCore a Corvo 0.4.0

LuxCore pasa a llamarse **Corvo**. Cambian los paquetes, las coordenadas de Maven, las claves de
...
### 1 · Instala Corvo 0.4.0

```bash
cd <el-repositorio-de-corvo>
./corvo install          # compila, prueba e instala en ~/.m2
```
...
Si tu aplicación usa `Migrations` de `corvo-data`, para y lee.

El framework anota en una tabla qué migraciones ya se aplicaron. Esa tabla se llamaba
`lux_migraciones` y ahora se llama `corvo_migraciones`. Si arrancas sin renombrarla, **Corvo no la
...
```sql
ALTER TABLE lux_migraciones RENAME TO corvo_migraciones;
```

Lo mismo con la tabla de sesiones, si usas `JdbcSessions` y le pasabas el nombre por defecto:

```sql
ALTER TABLE lux_sesiones RENAME TO corvo_sesiones;
```
...
| | Antes | Después |
|---|---|---|
| Paquetes | `import lux.core.Route` | `import corvo.core.Route` |
| Clase de arranque | `Lux.run(8080, …)` | `Corvo.run(8080, …)` |
| Servlet | `LuxServlet` | `CorvoServlet` |
| groupId | `lux` | `dev.ginit.corvo` |
| Artefactos | `lux-core`, `lux-data`… | `corvo-core`, `corvo-data`… |
| Versión | `0.2.x` / `0.3.x` | `0.4.0` |
| Propiedad del pom | `${lux.version}` | `${corvo.version}` |
| Claves de config | `lux.oauth.google.id` | `corvo.oauth.google.id` |
| Variables de entorno | `LUX_SERVER_PORT` | `CORVO_SERVER_PORT` |
...
`LUXSESSION` pasa a ser `CORVOSESSION`. El navegador de cada usuario sigue mandando la vieja, que
...
| Antes | Después |
|---|---|
| `lux_requests_total` | `corvo_requests_total` |
| `lux_request_errors_total` | `corvo_request_errors_total` |
| `lux_request_duration_ms` | `corvo_request_duration_ms` |
| `lux_uptime_ms` | `corvo_uptime_ms` |
| `/lux/metrics` | `/corvo/metrics` |
| `/lux/metrics/prometheus` | `/corvo/metrics/prometheus` |
...
Era la opción evidente: que Corvo entendiera los dos nombres durante una temporada. Se descartó a
propósito.
...
```sql
ALTER TABLE corvo_migraciones RENAME TO lux_migraciones;
ALTER TABLE corvo_sesiones RENAME TO lux_sesiones;
```
...
- [ ] `./corvo install` — Corvo 0.4.0 en `~/.m2`
- [ ] El árbol de git de la aplicación, limpio
- [ ] `ALTER TABLE lux_migraciones RENAME TO corvo_migraciones;`
- [ ] `ALTER TABLE lux_sesiones RENAME TO corvo_sesiones;` (si aplica)
- [ ] `./corvo migrar ../mi-app --probar`
- [ ] `./corvo migrar ../mi-app`
- [ ] `git diff` y `mvn test`
